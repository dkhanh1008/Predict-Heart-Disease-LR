...
#st.write('Kindly note that the results generated by this model are intended solely for educational and learning purposes. \
...
st.write('Please note that this model has limited predictive power (AUC=0.65 on the test dataset) and favors False Positive (Type I) errors. \
         Hence, the generated results are for educational and learning purposes only and should not be relied upon for any practical or decision-making purposes')
st.divider()

#logistic regression - sigmoid function
st.subheader('Logistic Regression Model - Sigmoid Function:')
st.latex(r'Supposed\;true\;model:\;  logit^{-1}(x) = \frac{1}{1 + e^{-x}} =  \frac{1}{1 + e^{-(\beta_0 + \beta_1 male + \beta_2 age + \beta_3 cigsPerDay  + \cdots + \beta_{12} heartRate + \epsilon)}}')
st.write(" ")
